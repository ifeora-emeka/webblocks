  ...
  return (
    <>
      <Box
        opacity={1}
        className={cn(
          'element_toolbox bg-card absolute p-1 rounded-md shadow-xl border z-[1000]  min-h-8 border-border text-muted-foreground flex items-center gap-default_spacing_lg justify-between opacity-100 hover:opacity-100 ',
          {
            'top-5 right-5': !parentID,
            '-top-10 left-0': parentID,
          },
        )}
        onClick={(e) => {
          e.stopPropagation()
